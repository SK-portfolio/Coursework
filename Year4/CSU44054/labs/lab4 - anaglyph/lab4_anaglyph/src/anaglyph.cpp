...
// Anaglyph control 
static float ipd = 3.0f;				// Distance between left/right eye.
	...
	do
	{
		...
		if (anaglyphMode == None) {
			...
			if (anaglyphMode == ToeIn) {
				...
				// ------------------------------------------------------------
				glm::vec3 lookDirection = glm::normalize(lookat - eyeCenter);
				float eyeConvergenceDistance = glm::dot(lookDirection, eyeCenter - lookat);

				float leftAngle = atan(ipd / (2.0f * eyeConvergenceDistance));
				float rightAngle = -leftAngle;

				glm::mat4 toeInLeft = glm::rotate(glm::mat4(1.0f), leftAngle, up);
				glm::mat4 toeInRight = glm::rotate(glm::mat4(1.0f), rightAngle, up);

				glm::mat4 viewMatrixLeft = glm::lookAt(eyeCenter, lookat, up) * toeInLeft;
				glm::mat4 viewMatrixRight = glm::lookAt(eyeCenter, lookat, up) * toeInRight;
				...
				// ------------------------------------------------------------
				glm::vec3 lookDirection = glm::normalize(lookat - eyeCenter);
				float distance = glm::distance(eyeCenter, lookDirection) * 9;
				...
				float left;
				float right;
				float top = zNear * (h / (2 * distance));
				float bottom = -zNear * (h / (2 * distance));

				left = -zNear * ((w - ipd) / (2 * distance));
				right = zNear * ((w + ipd) / (2 * distance));
				// Create asymmetric frustum projection matrices for left and right eyes
				glm::mat4 projectionMatrixLeft = glm::frustum(left, right, bottom, top, zNear, zFar);

				left = -zNear * ((w + ipd) / (2 * distance));
				right = zNear * ((w - ipd) / (2 * distance));

				glm::mat4 projectionMatrixRight = glm::frustum(left, right, bottom, top, zNear, zFar);

				// Calculate the direction vectors for the left and right eyes
				glm::vec3 leftEyeOffset = -(ipd / 2) * glm::normalize(glm::cross(lookat - eyeCenter, up));
				glm::vec3 rightEyeOffset = (ipd / 2) * glm::normalize(glm::cross(lookat - eyeCenter, up));

				// Calculate view matrices for left and right eyes
				glm::mat4 viewMatrixLeft = glm::lookAt(eyeCenter + leftEyeOffset, lookat, up);
				glm::mat4 viewMatrixRight = glm::lookAt(eyeCenter + rightEyeOffset, lookat, up);
