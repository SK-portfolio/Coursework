...
unsigned long hash2(char *str) {                //djb2 found in http://www.cse.yorku.ca/~oz/hash.html
...
/*int hash2(char* key) {
    int hash_value = 0;
    while (*key) {
        char normalized_char = (*key >= 'a' && *key <= 'z') ? (*key - 'a' + 'A') : *key;
        hash_value += normalized_char;
        key++;
    }
    return hash_value % ARRAY_SIZE;
}*/

void insert(struct Surname* hash_table[], char* name) {                                 //inserts name into hash table
    int index = hash2(name);                                                            //calculates the hash value of given name
...
void Input2Frequency(struct Surname* hash_table[]) {        //reads all inputs until 'quit' is entered
    ...
    while (strcmp(name, "quit") != 0) {                     //~while 'quit' has not been entered
        int index = hash2(name);                            //get hash index
        struct Surname* curr_name = hash_table[index];      //find current name occupying index
...
int main() {
    ...
    printf("\tCapacity\t: %d\n", ARRAY_SIZE);
    printf("\tNum Terms\t: %d\n", termCount);
    printf("\tCollisions\t: %d\n", collisionCount);         //goes from 10 collisions to 8
    printf("\tLoad\t: %.2f%%\n", load);
